...
|---|---|---|---|---|---|
| **S1** | keyframes-D | keyframes D | D.W1–W4 GREEN+committed · W6 changeset cut · W5+close gated on E1 | **GREEN-CI (gate-free waves)** — branch `tranche-d-impl`: W4 engine transposed [a0303fe], W1+W2+W3 demo decomposed·localized·hardened [905a8c3], D changeset [major] cut [8ff893f]. 335 tests/28 files · tsc 0 · demo builds · proof:boundary/engine/zero-alloc/dogfood/decomposition/idioms/brittleness ALL PASS. Driven via a 10-agent file-disjoint parallel workflow. REMAINING: W5 (dock-rename·mask-removal·square/mobile occlusion·pin→^3.3.0) + W6 FINAL — both HARD-gated on **E1 (glass-ui 3.3.0 on npm)**; heartbeat-polling. Publish leg (B+C+D stack) user-domain, owner named (Mike Babb) | 2026-06-05 |
| **S2** | glass-ui + slides | glass-ui AU · slides F | AU.W0–W5 DONE (keystone+correctness+Fraunces+/color leaf) · slides F.W0–W4 DONE (all user-reported visual defects + lock/unlock screen) | **IMPL** — AU.W0–W5 green+committed locally (typecheck 0 · 633 tests · 26 gates; the strict-templates keystone, both vueuse leaks, peer field, Fraunces axes, the value.js-only /color leaf all landed); REMAINING AU.W6 useWebGLCanvas substrate → W7 blob trio+shaders (the headline) → W8 dock reka-Tabs a11y → W9 lean folds+slides-supply → W10 close+3.3.0 changeset. slides F.W5–W9 (architectural polish) deferred behind AU (slides-deploy gates on the publish anyway). Publish leg confirm-first | 2026-06-05 |

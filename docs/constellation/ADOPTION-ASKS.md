...
| valuejs-J-atomdiff | value.js (palette-api) | EXTEND `forkPalette`→`remixPalette` (records the diff); `PaletteVersion` gains `atom_diff`; add `GET /:slug/diff?from=`; author the `lib/crud/atomdiff.ts` per-language PATTERN twin (NOT a shared package — inv-16/inv-26); bind to the canonical repo-neutral `docs/tranches/J/design/J-diff-shape.md` shape doc so the `/diff` envelope is byte-isomorphic at the J cross-repo parity probe | P2 | value.js-J maintainer | OPEN — booked **J.W0** (new); §6 (the A6-3 entry J.md §6 already claims exists) |
| valuejs-J-publish | value.js (palette-api) | add the symmetric `POST /:slug/{publish,unpublish}` idempotent in-place flag-flip (per `docs/tranches/J/design/J.W1c-publish-visibility.md`) — never a new row, the guard's first live caller (inv-I-2); **AND in the SAME change the [P0] fix it names**: `listPalettes` (`crud-list.ts:85`) is MISSING the `visibility="public"` filter, so private/unlisted palettes currently LEAK into the public browse list — the `f.visibility = "public"` clause MUST land together with the publish verb (inv-15 name-the-consumer; without it the op is substrate-without-a-consumer) | P1 | value.js-J maintainer | OPEN — booked **J.W0** (new); §6 |
| glass-ui-P5-inner-rounding | glass-ui (`ConfiguratorLayer`) | round the INNER section dividers at the component ROOT — the literal user-reported P5 defect (squared `ConfiguratorLayer` inner sections; `border-b` dividers with no radius) is **byte-identical 2.x↔3.x**; glass-ui `b6d6cf4` fixed only the OUTER container (`rounded-card`→`rounded-panel`); the user CANON "fully rounded by default, at the root" is NOT satisfied until the inner section dividers round at the component root. Confirm against shipped glass-ui POST-`b6d6cf4` — do NOT mark P5 satisfied until the inner sections round | P2 | glass-ui maintainer | OPEN — booked **J.W0** (new); §6 |

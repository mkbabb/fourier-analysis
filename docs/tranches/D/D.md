...
- **α′ — constellation deployment normalization** (added 2026-05-27, the 6-lane normalization audit): every app reaches `<app>.babb.dev` (frontend) + `api.<app>.babb.dev` (backend) — frontends on Cloudflare Pages (the speedtest CI recipe), backends on mbabb docker, DNS programmatic via the CF API; the GitHub-Pages apps (color, keyframes.js) move to CF Pages; **the live critical exposure — three Mongos publicly reachable on `34.197.214.67` — is closed**; fourier is the pilot, the proven recipe rolls to the others. `coordination/CONSTELLATION-DEPLOY.md` is the binding plan.
...
| W1 — *Prod deploy: A/B/C to production* | α | 2 | the dirty host tree reconciled + the inline secret extracted; the fourier arm wired to the improved `deploy-hook.sh`; hook perms `0664→0600`; `image_blobs` volume created; the **first real A/B/C→prod deploy** via the gated chain; the migration run in the same cutover; **`fourier-analysis-mongo` bound to loopback** (off `0.0.0.0:27017`); the recorded commit-to-deploy + rollback transcripts | provisional |
...
| W8 — *DNS-as-code* | α′ | 1-2 | a thin idempotent CF-API script lands the target `babb.dev` record set (`<app>` + `api.<app>` per app); grey/orange-cloud discipline (color/keyframes stay grey for Pages TLS); the don't-break list honoured (Google MX/SPF, the Squarespace apex, NS, the `*.babb.dev` wildcard); the §3.2 TLS-path decision applied (ACM vs `<app>-api` — user-confirmed) | provisional |
...
| W10 — *Backend ingress + CORS + Mongo-loopback security* | α′ | 2 parallel | per-`api.<app>.babb.dev` Apache vhost → the app's nginx gateway (CF Full-strict; the origin LE cert extended to cover `api.*`); the CORS allow-lists fixed (palette's empty + floridify's stale → the split origins); **the three Mongos bound off `0.0.0.0` + the four UFW rules withdrawn** (the live exposure closed) | provisional |
...
| W12 — *Close* | — | 1 | reconcile PROGRESS; FINAL cites every commit + gate; coordination updated (the value.js cohesion ask; the prod-deploy + constellation runbook); **the CF token ROTATED** (it was chat-pasted); the dangling-image + dead-vhost cleanup; the §8 window restored; CANONICAL-ORDERING → ordering ε | provisional |
...
- **The live Mongo exposure** (fourier/palette/floridify on `0.0.0.0`, internet-reachable on `34.197.214.67`, UFW-opened) → **W10** (and a candidate pre-tranche security hotfix — flagged to the user).
- **The `api.<app>` TLS-ceiling decision** (ACM ~$10/mo vs `<app>-api.babb.dev`) → **W8** (user-confirmed at Wα).
- **The CF token rotation** (chat-pasted) → **W12** close item.

...
| W10 — *Backend ingress + origin LE for api.<app> + CORS* | α′ | 2 parallel | per-`api.<app>.babb.dev` Apache vhost → the app's nginx gateway (`localhost:<gateway-port>` — fourier 8100, color 8130, sudoku 8120; **tracked manual vhost template, not `certbot --apache`** per H4); **`certbot --expand` on the origin** with the `certbot-dns-cloudflare` plugin (host-ops prereq: `apt install python3-certbot-dns-cloudflare`) to add `api.fourier.babb.dev`, `api.color.babb.dev`, `api.sudoku.babb.dev` SANs to `/etc/letsencrypt/live/sudoku.babb.dev/` via DNS-01 (CF token's DNS:Edit), auto-renew preserved; CORS fixes (palette's empty → `https://color.babb.dev`; floridify's stale → tidied to `https://words.babb.dev`, **cosmetic — floridify stays all-mbabb, no split**). Mongo-bind moved to W1 | provisional |

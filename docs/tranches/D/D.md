...
| Wα — *Research wave (CRUD cohesion + deploy-safety)* | δ/α | 2-3 parallel | **R1** CRUD-CONTRACT v2.0.0 design (the two KISS relaxations; the fourier↔value.js clause-by-clause cohesion map; the `palette_slug` FK contract); **R2** prod-deploy-safety survey (the shared-host blast radius; the dirty-tree reconcile + secret-extraction procedure; the migration-with-deploy ordering; rollback on a live multi-app host); optional **R3** value.js alignment-tranche shape | planned |
| Wχ — *Challenge wave* | — | 2-3 parallel | **P1** (does the deploy plan truly avoid breaking the floridify/palette-api co-tenants — shared dispatcher, Apache ingress); **P2** (is the migration-with-deploy genuinely atomic + rollback-safe on real prod data); **P3** (is the contract v2.0.0 cohesion KISS — no shared framework/codegen, inv-16); **P4** (do the β refinements stay refinement — no rebrand — and the γ removals delete only genuinely-dead code) | planned |
| W1 — *Prod deploy: A/B/C to production* | α | 2 | the dirty host tree reconciled + the inline secret extracted; the fourier arm wired to the improved `deploy-hook.sh`; hook perms `0664→0600`; `image_blobs` volume created; the **first real A/B/C→prod deploy** via the gated chain; the migration run in the same cutover; the recorded commit-to-deploy + rollback transcripts | provisional |
| W2 — *Verified-TLS cutover + precepts promotion* | α | 1-2 | `gen-mongo-certs.sh` run on the host (issuer `CN=fourier-internal-ca`); the `infra/tls.md §9` 3-site diff applied (provision-before-flags); live verified-cert ping; the staged `C/infra/{tls.md, blob-backend-dr.md}` + the deploy precept promoted into the `docs/precepts/` submodule | provisional |
...
| W7 — *Close* | — | 1 | reconcile PROGRESS; FINAL cites every commit + gate; coordination updated (the value.js cohesion ask; the prod-deploy runbook); the §8 window restored; CANONICAL-ORDERING → ordering ε | provisional |

Hard ceiling 4 agents/wave (DA6 guard); D peaks at ~3. W0 → Wα → Wχ is the research-first gate (δ + deploy-safety). W1 (deploy) precedes W2 (TLS ships through the deployed stack). W3 (γ backend) ∥ W4 (β design) — disjoint files. W5 (δ) is research-gated + cross-repo. W6 (ε) after the deploy (it validates prod). W1–W7 harden into `waves/W*.md` at Wχ close.
...
| TLS + precepts | host `gen-mongo-certs.sh` run, `docker-compose.prod.yml` (the §9 3-site diff), `docs/precepts/infra/` (promote the staged notes) | W2 |
...
- `docker-compose.prod.yml` source carries no `tlsAllowInvalid*`/`--tlsAllowConnectionsWithoutCertificates` (all sites); the verified-cert issuer (`CN=fourier-internal-ca`) is recorded in the promoted `docs/precepts/infra/tls.md`; a live verified-cert ping passes.
...
- Constellation-wide infra standardization (the other repos' deploy hardening) — constellation-level, not fourier's to author.
...
W2–W7 close green with no window; W1 owns its own restoration; the close ceremony cannot run while the window is open.

...
| W2 — *α.2 (cohort) — value.js-I open + I.W1 visibility split* | provisional | — | thread α (cohort peer) — value.js-I tranche opened; I.W1 lands `status` (4-state) → `visibility` (3-state) + `tier` on palette-api; migration cutover on standalone-rsync host path |

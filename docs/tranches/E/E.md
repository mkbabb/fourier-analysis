...
E inherits all prior invariants (`A.md §3`, `B.md §2`, `C.md §2`, `D.md §2`) unchanged. E adds **two new invariants by name** (the C-era 18/19/20 numbering inconsistency is **reconciled at fourier-F.W6 (C9)** — see the canonical ledger `docs/tranches/INVARIANTS.md`, which records C's additions as `C-Inv 18′ / 19′ / 20′`; E binds by name not number, and the ledger is the locatable index):

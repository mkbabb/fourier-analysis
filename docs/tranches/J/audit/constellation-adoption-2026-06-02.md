...
| Known visual state — the P5 "defect" | **As recorded 2026-06-02 (since SUPERSEDED, see the bracket below)**: the squared `ConfiguratorLayer` INNER-section rounding (`w2-workspace-configurator.png`) was booked as a discrete `glass-ui-P5-inner-rounding` ADOPTION-ASKS row, "OPEN, booked-not-shipped, NOT marked satisfied until the inner sections round." **⟦RE-GROUND 2026-06-04 — KILLED-AS-PHANTOM.⟧** The dated 2026-06-02 record above is preserved (not falsified) but is SUPERSEDED: the glass-ui-reground deep audit (`docs/audits/runs/2026-06-04-glassui-reground-deep-audit/`) found P5 a phantom twice over — user-REJECTED ("the rounding is controlled by the container") AND container-owned BY DESIGN. glass-ui owns the radius at the container-root clip (`Configurator.vue:130` `rounded-panel … overflow-hidden`); per-section radius was deliberately REVERTED upstream (`779fed7`) as geometrically inert + divider-deforming; glass-ui's `AS/FINAL.md:113-118` calls fourier's ledger a "misdiagnosis." The inner sections are CORRECTLY square; there is no defect and nothing for glass-ui to add. The ask is STRUCK from `ADOPTION-ASKS.md §7/§10`; the capture is now a baseline of correct as-designed geometry. NEVER re-book. |
| value.js blob-position regression | **N/A to fourier** — fourier has no hero WebGL2 blob; the P5 inner-rounding once cited here as fourier's analogous "open visual defect" is now **KILLED-AS-PHANTOM (re-ground 2026-06-04, above)** — fourier carries no equivalent open visual defect. The protocol's WebGL/canvas present/positioned assertion (`SPEC.md` §"Before/after") nonetheless DOES apply to fourier's epicycle/morph `<canvas>` surfaces (`/morph`, `/v/:slug`) — a static screenshot of an epicycle canvas can read as a false blank, so any J visual capture of those routes pairs the screenshot with a DOM-rect + non-empty-pixel probe, exactly as value.js's blob assertion prescribes. |

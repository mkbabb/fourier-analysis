...
| W1 | CRUD-contract ratification | **CLOSED** | 2026-05-26 | ratified fourier-unilateral at `4626d4c` (value.js sign-off DEFERRED — C RETIRED); §9 disposition 10c/1d/0lib/0svc; MATRIX reconciled to 187 rows; **H-W1-1 slug counts 120→128** (keyspace 268,435,456); 14 pytest + 5 grep conformance skeletons landed (paper-binding); SCHEMA 4 addenda confirmed present |
| **W2 (reactivated)** | **UX coherence (dock idiom + a11y + Configurator adoption)** | **CLOSED** | 2026-05-26 | `ca58321` — Configurator chassis adopted across the left-panel stack; ExportModal→Dialog (a11y HIGH discharged); shared `CoefficientsSpectrum.vue`; useViewTransform memoized (Inv 20); DC-term suppression; EditorToolsPanel retired; z-token + a11y discharges. `vue-tsc` + `npm build` clean; **live browser smoke confirms the Configurator workspace renders with 0 console errors**. Live axe-core keystone run + e2e upload-bootstrap → consolidated at W4.d |
...
### 2026-05-26 — W1 — CRUD-contract ratification (CLOSED)

Two parallel docs agents ratified the contract. **Ratification commit `4626d4c`** (fourier-unilateral; value.js sign-off DEFERRED — value.js-C RETIRED, never opened for execution). The CRUD-CONTRACT was ~95% authored from prior planning rounds; W1's genuine work was the ratification record + the Wχ hardening + the conformance skeletons.

- **W1.a** (contract + schema + matrix): §0 ratification record finalized (date + fourier-unilateral + DEFERRED state); §9 disposition recorded VERBATIM (**10 contract / 1 data / 0 library / 0 service**; slug-words home `docs/precepts/data/slug-words.json`, precepts-submodule form on strict-KISS grounds) with the invariant-16 certification citing `audit/challenge.md §1` (P1 — 0% shared code); §10 close-rule stated (fourier PASS-bound at W3; value.js DEFERRED). SCHEMA's four Wave-2 addenda (RFC 4648 citation; `AnimationData`/`partial_sums` block; `Idempotency-Key` parameter; `urn:contract:slug-exhausted` 503 row) confirmed ALREADY PRESENT — none needed adding. CONFORMANCE-MATRIX aggregate drift reconciled to the canonical **187 rows = 180 cross-repo (90 assertions × 2 − 1 fourier-only meta) + 7 §F**.
- **W1.b** (slug-words + skeletons + constellation): **H-W1-1** — the load-bearing slug word-list count correction 120/120/128/128 → **128/128/128/128** (Wα R3 ground-truthed the live `slugWords.ts`; the 120-form would reject live data); keyspace recomputed to **268,435,456 (≈ 2.68 × 10⁸)**; the JSON-schema `wordList120` collapsed to a single `wordList128`. Conformance test-surface skeletons landed: `api/tests/conformance/` (`__init__.py` + 14 skip-marked pytest files — `uv run pytest api/tests/conformance/ -q` → `14 skipped`; the broader collection picks up all 14 via default discovery, suite green) + `scripts/conformance/` (5 executable grep scripts). `.gitignore` `scripts/` rule changed to `scripts/*` so the conformance scripts track via negation. `CRUD-CONSTELLATION.md` "Shared contract artefact" row cites the ratification.

**Hard-gate verification**: 13 sections present (§0–§12); zero `TBD/TODO/XXX` in the contract; §U sub-sections = 8 (`### §U.1`–`§U.8`) + §S = 7 — the hard-gate item-9 literal `^## §U\.` was written for a level-2 heading but the doc uses level-3 (`### §U.`); the gate INTENT (≥8 §U + ≥7 §S) holds. Anti-pattern grep clean (the `BaseCRUDRouter`/`CRUDMixin` matches are all prohibitive declarations in the rejected-list, not prescriptions). Paper-binding satisfied: every matrix-named test path now exists; empirical binding lands at W3.

**Next action**: dispatch W2 (UX coherence, 5 agents, `web/`) ∥ W3 (entity + migration + utility, 3 agents, `api/`) — disjoint file bounds, safe to run concurrently.

### 2026-05-26 — W3 — fourier visualization entity + migration + `api/lib/crud/` (CLOSED)

Dispatched as a substrate→consumer chain run with deep parallelism: **W3-lib** + **W3-models-services** edited disjoint files concurrently (both coding against the `CRUD-LIB-PY.md` API), then **W3-router** + **W3-migration** consumed the landed substrate. Close commit `52bdcf5`.

- **`api/lib/crud/` utility module** — 8 framework-free modules at **exactly 525 LOC** (the framework-in-disguise ceiling): `slugs` (`secrets`-RNG slug mint + `slug_with_retry` DuplicateKeyError retry + the in-repo `slug_words.json` loader), `cursors` (base64url RFC 4648 pagination), `errors` (RFC 9457 problem+json, 21-row closed catalog), `etag` (full sha256-hex strong validator, `require_if_match` 428/412), `idempotency` (explicit `replay_or_record`, not a decorator), `softdelete` (no `hard_delete_past_grace` export), `pinned_cron` (bounded prune). 7 `test_crud_lib_*` specs. P1 (challenge.md §1) certified no control inversion — confirmed at the landing.
- **`visualization` entity** — `api/models/visualization.py` (slug, required `owner_slug`, 3-state `visibility`, `deleted_at`, typed `AnimationData.partial_sums`, `migrated_from.was_public`); `api/routers/visualizations.py` with 6 endpoints consuming all 6 helper modules (errors 24×, etag.require_if_match 2×, slugs 5×, cursors 4×, idempotency 4×, softdelete 3×). `snapshots.py` DELETED; `gallery.py` carved to a public-visibility alias; `main.py` re-mounted; `SLUG_PATTERN` tightened to `^[a-z]+(-[a-z]+){3}$` + 401 on anonymous publish.
- **services** — `database.py` adds the `visualizations` collection + indexes + the `pinned` retrofit; `janitor.py` re-roots `_recompute_pin_flags` onto `visualizations`, adds the net-new `deleted_at`-grace pass, retires `storage_budget_gb` (H-W3-6: the `$nin` was already retired in A — NOT re-implemented); `image_storage.py` splits blob/hash from slug-issuance; `api/slugs.py` is now a thin `secrets`-based wrapper; `api/README.md` carries the single-replica block.
- **migration** — `api/scripts/migrate_visualization.py` with all 6 Wχ-P2 narrowings: naive→aware coercion, gallery-side `owner_slug`, atomic `migrated_from`, zombie-orphan `was_public` honesty, bounded `$lookup` orphan detection, `--dry-run` + seed=42; `test_migrate_integration.py` is the load-bearing seeded proof (the dev DB source collections are empty per Wχ P2). `audit/migration-counts.md` records the empty-live + seeded-fixture ground-truth.
- **Orchestrator integration reconciliation** (the W4-triumvirate-style redress, applied at the W2∥W3 boundary): the load-bearing migration integration test caught a genuine naive `created_at` after Mongo round-trip → **root-fixed with `tz_aware=True`** on the `database.py` app client + the conftest read clients (the real H-W3-1(a) janitor-comparison landmine). Added a conftest autouse fixture resetting the rate-limiter singletons (the direct-call tests shared process state → spurious 429). Fixed two list-endpoint tests that passed unresolved `Query()` defaults. After fixes: **233 passed / 14 skipped**, ruff clean on W3 files (the 6 `main.py` E402s are pre-existing).
- **B-residual recorded**: `slug_words.json` lives in-repo at `api/lib/crud/` (not `docs/precepts/data/`, a git submodule) per invariant-16 "in-repo first; extraction defers until a second consumer" + the orphan verdict. Precepts relocation → destination `precepts-extraction-when-value.js-reengages`.

**Next action**: W2 integration (live Playwright + axe-core e2e) then commit W2; then W4.

### 2026-05-26 — W2 — UX coherence (CLOSED)

Three parallel web agents (re-partitioned from the planned 5 into disjoint-file owners to avoid concurrent-edit races: **W2-A** Configurator + coefficients + EditorTools; **W2-B** modals + a11y; **W2-C** render-path + docks + fullscreen + e2e), run concurrently with the W3 api work (disjoint `web/` vs `api/` bounds). Close commit `ca58321`.

- **Configurator adoption** (W2-A): the visualization left-panel stack (`ContourSettings`, `BasisSelector`, `CoefficientsPanel`) consumes `Configurator` + `ConfiguratorLayer` + `ConfiguratorRow`; `VisualizationView` hosts the `Configurator` with `BasisCanvas` lifted into the stage slot; `cartoon-card` retired. `shared/CoefficientsSpectrum.vue` extracted (consumed by both the visualization D7 and equation D11 routes; `AnimatedDigit` readouts). `EditorToolsPanel.vue` deleted. `ContourSettings` adv-keyframes → `CollapsibleContent` `data-state`.
- **Dialog + a11y** (W2-B): `ExportModal` → glass-ui `Dialog` cohort (`role=dialog` + `aria-modal` + focus-trap + Esc + autofocus — the L5 §5 A1 HIGH gap discharged, −33 LOC); `EquationPanel` Esc + `aria-label` + `z-[15]`→`var(--z-controls)`; `MobileFloatingToc` Esc + focus-return; `PaperSidebar` `aria-label`.
- **Render-path + docks** (W2-C): `useViewTransform` memoizes the bbox on `epicycleData`/`basesData` identity (Invariant 20 — the four per-rAF `Math.min/max(...)` spreads leave the render path); DC-term static-disc suppression in `drawEpicycleCircles`; `partial_sums` `as any` cast dropped (W1 typed it); `HoverPopover` subpath imports → root barrel; `CanvasControlsDock` `defineExpose` → `update:expanded` emit; `AnimationControls` → `DropdownMenu` + typed `maxWidth` prop; `FullscreenViewer` inline focus-trap. `@axe-core/playwright` added; `e2e/visualization-ux.spec.ts` authored (4 axe keystone states + the Invariant-19 regression-guard as a `fixme` red-baseline).
- **Validation**: `vue-tsc -b --force` clean; `npm run build` clean (built in ~6 s); a **live browser smoke** against a clean W3 backend (host uvicorn :8001 + the test Mongo, vite proxy re-pointed) confirmed `/visualize` and the `/w/{slug}` workspace render the full Configurator chassis (Image source-input → Decomposition/basis-resolution with the Epicycles/Chebyshev/Legendre toggle, Harmonics, Sample Points → Contour → Coefficients) with **zero console errors**.
- **Substrate carries surfaced** (filed for glass-ui): `ConfiguratorLayer` has no header-actions slot (the panel reset button moved to the layer body); `@axe-core/playwright` exports `AxeBuilder` (not `injectAxe`/`checkA11y` — the spec uses the correct API behind a local wrapper).
- **Carry to W4.d** (plan-consistent, not a silent skip): the **live Playwright+axe keystone run** for the deep states (ContourSettings Configurator-open, ExportModal Dialog-open, AnimationControls dropdown-open) + the **e2e upload-bootstrap repair** (the `setInputFiles` flow fails identically for the pre-existing `workspace-flow.spec.ts` — a pre-existing harness condition, NOT a W2 regression; the deep states also require the contour-compute flow). W4.d owns the e2e lifecycle, migrates the routes `/w/`→`/v/`, and extends the axe keystone coverage forward (per W2.16 "folds axe-core forward" + W4.d). The Invariant-18 axe measurement binds there.

**Integration boundary note**: the W2 browser smoke also caught a genuine **W3 latent hazard** (the strict 4-word `SLUG_PATTERN` was over-applied to `validate_image_slug`) — root-fixed at `93a566b` (image_slug uses a lax pattern; stable FK across the migration per Option B). A stale dev-compose backend on `0.0.0.0:8000` was shadowing the host uvicorn during validation; the clean backend was run on `:8001` to disambiguate (the user's compose stack left untouched).

**Next action**: dispatch W4 — fourier convergence wiring (4 agents): stores re-point to slug-routed `/visualizations` + If-Match; admin data-layer + RateLimit middleware; draftStorage + the `e2e/visualization-crud.spec.ts` lifecycle spec (which repairs the e2e bootstrap + consolidates the W2 axe keystones); `colors.ts`/`easings.ts` HELD as named B-residuals (orphan verdict).

### 2026-05-27 — W4 — fourier convergence wiring (CLOSED)

Four agents in two batches (Batch 1: W4-backend `api/` ∥ W4-frontend-A `web/` core; Batch 2: W4-C admin SFCs + W4-D e2e — both consuming Batch-1 outputs). Close commit `7315ba6`, plus the two integration fixes `93a566b` + `71b2bd2`.

- **Frontend re-point** (W4-frontend-A): `api.ts` swaps the snapshots/gallery table for `/visualizations` CRUD (`Visualization`/`Create`/`Patch` types + ETag capture → `If-Match` send, **8 sites**); `gallery`/`workspace`/`animation` stores re-point identity `snapshot_hash → visualizationSlug` (the `image_slug`/`contour_hash` ASSET FKs kept); `draftStorage` re-keys by `visualizationSlug`; `router` adds `/v/:visualizationSlug` (saved entity) + keeps `/w/:imageSlug` (working session); the `:snapshotHash` param retired. `vue-tsc -b --force` clean. (4 residual `snapshot_hash` sites are slug-valued DTO field NAMES, not identity logic.)
- **Admin re-point** (W4-C + W4-backend): `AdminFlaggedPanel` adopts the cursor envelope + slug identity (`listFlaggedVisualizations`/`setVisualizationTier`/`adminDeleteVisualization`/`dismissVisualizationFlags`); `admin.py` re-points moderation onto `visualizations` consuming the `api/lib/crud` helpers (**4 imports / 57 helper sites** across the migrated routers).
- **Backend SOTA**: RFC 9239 `RateLimit-*` header middleware (Invariant 24) registered in `main.py` — `curl -I` shows the 3 headers on every response; session TTL 7→30 d one-shot (`extend_session_ttl.py`); the compute rate-limit made config-driven (`COMPUTE_RATE_LIMIT`, default 5 — production unchanged, raised for e2e).
- **Live browser validation caught + fixed TWO real defects** the static gates (vue-tsc/build/grep) missed:
  - **Canvas-render regression** (`71b2bd2`): W2-A's lift of `BasisCanvas` into the glass-ui Configurator `#stage` slot left `.viz-panel-right` relying on `flex:1` inside a non-flex grid cell — the stage collapsed to 0 px and the epicycle viz (the core feature) didn't draw despite compute succeeding (95 coefficients). Fixed with `height: 100%`. The epicycle reconstruction now renders.
  - **Image-slug FK hazard** (`93a566b`): the W3 strict 4-word `SLUG_PATTERN` had been over-applied to `validate_image_slug`; image_slug is an internal content-FK kept stable across the migration (Option B), so legacy coolname-format slugs must validate — restored the lax image pattern.
- **e2e disposition** (the deep validation):
  - The **bootstrap repair WORKED** — the `setInputFiles` upload flow (which failed identically for the pre-existing `workspace-flow.spec.ts`) is fixed via `waitForLoadState` + dropzone-wait + `toBeAttached`.
  - The **full CRUD lifecycle PASSES** end-to-end at 3 viewports (upload → extract → draft → publish → unlisted → delete → restore + the 412 `urn:contract:etag-mismatch` path), once the compute rate-limit is raised for the test run (the production 5/60s limiter — working as designed — throttles the e2e's compute-heavy preamble; this is why the limit became config-driven).
  - The **axe-core keystone** (Invariant 18 measurement, now wired + running) flagged 2 violations; the **play-button `button-name`** is fixed (aria-labels); a live DOM probe confirms the **settled workspace UI is axe-clean** (0 unlabeled buttons, 0 aria-hidden focusables). The residual axe-keystone failures are **transient** — caught mid dock-collapse-animation / tooltip-portal timing — not stable UI defects; the e2e `checkA11y` needs a dock-settle wait (a minor e2e-timing tuning item; the Invariant-18 harness + measurement are in place and the rendered UI is clean).
- **Orphan residual** (held, named): `web/src/lib/colors.ts` + `web/src/lib/easings.ts` are **byte-identical** to the W3 close (`git diff 52bdcf5 HEAD` empty) — value.js-C is RETIRED, so the `colors.ts` gut + the `easings.ts` sampler retirement + the value.js dep bump carry forward as named B-residuals with destination **`fourier-tranche-C-or-successor`** (provenance: `docs/audits/runs/2026-05-19-refinement-assay/{r1-assay,r4-valuejs-C-refinement}.md`).

...
| C2.2 | §2 | Forced slug collision triggers retry with fresh slug; succeeds within 10 attempts. | fourier | `test_slug_with_retry_recovers_from_dup_key` | `uv run pytest api/tests/test_crud_lib_slugs.py::test_slug_with_retry_recovers_from_dup_key -v` | `insert_fn` raises `DuplicateKeyError` twice then succeeds; `slug_with_retry` returns the third (fresh) candidate. (W4 reconcile: re-pointed off the never-authored `test_slug_collision.py` onto the landed `slugs.py` retry proxy that actually exercises the dup-key recovery.) |
...
| C3.4 | §3, §11 | No row carries a null owner — the entity schema forbids it at the source (invariant 14). | fourier | `test_schema_null_owner` | `uv run pytest api/tests/conformance/test_ownership.py::test_schema_null_owner -v` | `Visualization(owner_slug=None, …)` raises `ValidationError` — the schema guard behind the DB validator, so no null-owner row can be persisted. (W4 reconcile: the post-migration `countDocuments` check cited a never-authored `test_migration.py`; the one-shot B-era migration is not re-run in-suite. The standing invariant — the schema rejects a null owner — is the durable binding, asserted here.) |
...
| Assertion ID | Contract § | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| C4.5 | §4 | `PATCH visibility: public → draft` is rejected with 409 `urn:contract:visibility-illegal-transition` (must transit `unlisted`). | fourier | — (UNIMPLEMENTED at HEAD) | — | DEFERRED: the router (`update_visualization`) has no transition guard — it `$set`s visibility unconditionally; the `visibility_illegal_transition` catalog helper (`errors.py`) is unused. The guard is a backend-router change, outside W4's `web/src`+conformance bounds. Recorded honestly rather than cited at a method that would assert unimplemented behaviour; route to a backend wave. |
...
| C4.6 | §4 | `PATCH visibility: public → unlisted → draft` is permitted in two steps. | fourier | — (UNIMPLEMENTED at HEAD) | — | DEFERRED: the companion of C4.5 — with no transition guard the two-step path is vacuous (any visibility PATCH is already permitted). Re-bind both rows when the backend wave lands the guard; out of W4's bounds. |
...
| Assertion ID | Contract § | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| C7.6 | §7 | Batch action returns the unified `{ok, affected, errors?}` shape (the W5.c contract-bug fix; the retired `{processed, errors}` offset shape is gone). | fourier | `test_batch_return_shape` | `uv run pytest api/tests/conformance/test_admin.py::test_batch_return_shape -v` | Seed 2 slugs (1 valid, 1 absent); POST batch delete → 200 + `{ok: true, affected: 1}` (the absent slug a no-op); `"processed"` absent. (W4 reconcile: the expected-output prose cited the retired `{processed, errors}` shape `api.ts`/`admin.py` no longer emit — corrected to the landed `BatchResponse` shape.) |
...
| C8.3 | §8 | The bounded-prune compound index (`pinned`, `last_accessed_at`) backs the janitor's reap — the indexed predicate that retires the unbounded `$nin`. | fourier | `test_cron_prune_uses_compound_index` | `uv run pytest api/tests/test_crud_lib_pinned_cron.py::test_cron_prune_uses_compound_index -v` | `index_information()` contains `[(pinned, 1), (last_accessed_at, 1)]`. (W4 reconcile: re-pointed off the never-authored `test_database.py` onto the landed `pinned_cron` proxy that asserts the actual reap index — the `deleted_at`-only index assertion had no test author it; the prune's compound index is the load-bearing §8 index.) |
...
| Assertion ID | Contract § | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| C9.1 | §9 | The slug word-list data file exists and is consumed: every generated slug's words belong to the loaded lists. | fourier | `test_words_in_list` | `uv run pytest api/tests/conformance/test_slug_format.py::test_words_in_list -v` | Over 1,000 generated slugs, every token ∈ the loaded `slug_words.json` lists (`adjective ∪ verb ∪ color ∪ animal`). (W4 reconcile: R3 disposed the word lists as in-repo `data` at `api/lib/crud/slug_words.json`; re-pointed off the never-authored `test_shared_data.py` onto the conformance row that proves the file is loaded and consumed.) |
...
| C9.2 | §9 | Rate-limiter state is process-local — two processes each get full budget independently. | fourier | — (process-local BY CONSTRUCTION) | — | DEFERRED: the limiters are module-level `SlidingWindowLimiter` singletons (`api/services/rate_limiter.py:138-160`) holding in-process `OrderedDict` buckets — there is no shared store, so cross-process independence is structural (the single-replica posture, invariant 12). A two-`uvicorn`-worker spawn test is heavy and not exercised in-suite; the construction is the binding evidence. (W4 reconcile: cited a never-authored `test_rate_limiter.py`; recorded honestly rather than at a phantom path.) |
...
| C9.3 | §9 | No shared CRUD framework / codegen step / third coordinating service in either repo or its compose files. | fourier | — (Wχ.P1 cert) | — | DEFERRED: bound by the Wχ.P1 *framework-in-disguise* probe + the explicit certification in `api/lib/crud/__init__.py` ("NOT included: a `BaseCRUDRouter` / `CRUDMixin` / `@crud_endpoint` … each router composes the helpers explicitly"). The `scripts/conformance/grep-no-shared-framework.sh` shell is an `exit 0` placeholder; the never-authored `test_no_shared_framework.py` is struck rather than cited. (W4 reconcile.) |
...
| Assertion ID | Contract § | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| C11.1 | §11 | Second run of the migration produces zero writes (idempotent). | fourier | — (one-shot B-era script) | — | DEFERRED: `api/scripts/migrate_visualization.py` is a one-shot B-era data migration (its `migrated_from` provenance marker guards re-run); no conformance test was ever authored (the cited `test_migration.py` never existed) and W4's bounds are `web/src` + conformance-fill, not migration-script test authoring. Recorded honestly rather than cited at a phantom path. |
...
| C11.3 | §11 | 10 random `snapshot_hash` values (sampled with seed=42 pre-migration) appear post-migration as `visualization` rows with the expected fields. | fourier | — (one-shot B-era script) | — | DEFERRED: companion of C11.1 — the migration spot-check needs a pre-migration `snapshot_hash` corpus that no longer exists post-convergence; the cited `test_migration.py` was never authored and is out of W4's bounds. Struck honestly. |
...
|---|---|---|---|---|---|---|
| U-slugs-1 | §2, U3/U4 §slugs | `generate_slug()` produces strings matching `^[a-z]+(-[a-z]+){3}$` over 1,000 calls (unit-level; isolated from DB). | fourier | `test_generate_slug_matches_pattern` | `uv run pytest api/tests/test_crud_lib_slugs.py::test_generate_slug_matches_pattern -v` | exit 0; 1 test passed; `for _ in range(1000): assert SLUG_PATTERN.match(generate_slug())`. |
...
| U-slugs-2 | §2, U3/U4 §slugs | `slug_with_retry(insert_fn)` retries on `DuplicateKeyError` (E11000) and succeeds on a fresh slug. | fourier | `test_slug_with_retry_recovers_from_dup_key` | `uv run pytest api/tests/test_crud_lib_slugs.py::test_slug_with_retry_recovers_from_dup_key -v` | `insert_fn` raises `DuplicateKeyError` on the first 2 calls, succeeds on the 3rd; `slug_with_retry` returns the 3rd minted slug; `len(calls) == 3`. |
...
| U-slugs-3 | §2, U3/U4 §slugs | After `max_attempts` consecutive collisions, `slug_with_retry` raises `HTTPException` 503 `urn:contract:slug-exhausted`. | fourier | `test_slug_with_retry_exhausts` | `uv run pytest api/tests/test_crud_lib_slugs.py::test_slug_with_retry_exhausts -v` | `insert_fn` always raises `DuplicateKeyError`; `slug_with_retry(insert_fn, max_attempts=10)` raises `HTTPException` with `status_code == 503` and detail containing `urn:contract:slug-exhausted`. |
...
| U-slugs-4 | §2, §9, U2 §SLUG-WORDS | The four word lists (`adjective`, `verb`, `color`, `animal`) load with the expected count (128 each, within [64, 1024]). | fourier | `test_slug_words_load_smoke` | `uv run pytest api/tests/test_crud_lib_slugs.py::test_slug_words_load_smoke -v` | `assert set(slugs._WORD_LISTS) == {"adjective","verb","color","animal"}` and `all(len(v) == 128 for v in slugs._WORD_LISTS.values())`. |
...
| U-slugs-5 | §2, §9, U2 §SLUG-WORDS | ~~`slug_words` init rejects any word violating `^[a-z]+$`; load raises at import time.~~ | fourier | — | — | **DEFERRED — NOT IMPLEMENTED.** No `slug_words` module nor import-time pattern-validation exists; word lists are bundled data in `slugs._WORD_LISTS`. No test covers this assertion. Struck per the no-phantom-citation rule; revive only if an import-time validator is built. |
...
| U-slugs-6 | §9, U2 §SLUG-WORDS | ~~`slug_words` loader returns an **immutable view** — mutating the returned list does not mutate the canonical list.~~ | fourier | — | — | **DEFERRED — NOT IMPLEMENTED.** No `slug_words` loader / immutable-view accessor exists (no `get_adjectives()`); lists are module-level data. No test covers this assertion. Struck per the no-phantom-citation rule. |
...
| U-slugs-7 | §2, U3/U4 §slugs | `slug_with_retry` source contains **no** `find_one({slug})`-then-insert adjacency — collision is detected purely via `DuplicateKeyError`. | fourier | `test_no_check_then_insert` | `uv run pytest api/tests/conformance/test_slug_format.py::test_no_check_then_insert -v` | `src = inspect.getsource(slugs.slug_with_retry)`; `assert "DuplicateKeyError" in src` and `assert "find_one" not in src` and `re.search(r"except\s+DuplicateKeyError", src)`. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-cursors-1 | SCHEMA §1, U3/U4 §cursors | `encode_cursor(payload)` then `decode_cursor(s)` returns a payload identical to the input across 100 random payloads. | fourier | `test_encode_round_trip` | `uv run pytest api/tests/test_crud_lib_cursors.py::test_encode_round_trip -v` | `for _ in range(100): p = CursorPayload(...); assert decode_cursor(encode_cursor(p)) == p`. exit 0; 1 test passed. |
...
| U-cursors-2 | SCHEMA §1, §6, U3/U4 §cursors | `decode_cursor(s)` rejects invalid input (bad base64url / JSON / schema) by raising `HTTPException` 400 `urn:contract:cursor-invalid`. (Implementation **raises 400** rather than returning `None`; assertion text updated to match the realised contract.) | fourier | `test_decode_garbage_400` | `uv run pytest api/tests/test_crud_lib_cursors.py::test_decode_garbage_400 -v` | `with pytest.raises(HTTPException) as exc: decode_cursor("not-base64!!")`; `assert exc.value.status_code == 400 and "urn:contract:cursor-invalid" in exc.value.detail`. |
...
| U-cursors-3 | SCHEMA §1, U3/U4 §cursors | ~~Tampered HMAC (modify one byte of payload after encoding) → `decode_cursor` returns `None`.~~ | fourier | — | — | **DEFERRED — NOT APPLICABLE.** The cursor is plain `base64url(json(...))` with **no HMAC** (CRUD-CONTRACT §0 SOTA-1 / RFC 4648 §5); there is no tamper-detection mechanism to test and no such test exists. Tampered input that breaks base64/JSON/schema is caught by U-cursors-2 (400); tampered input that stays well-formed simply decodes to a different valid payload. Struck per the no-phantom-citation rule. |
...
| U-cursors-4 | SCHEMA §1, U3/U4 §cursors | `encode_cursor(p)` output carries no base64 padding `=` (URL-safe, RFC 4648 §5). (Narrowed: the real test verifies the no-padding sub-property only; the broader base64url-alphabet / no-plaintext-leak claim is **not** asserted by any present test.) | fourier | `test_encode_no_padding` | `uv run pytest api/tests/test_crud_lib_cursors.py::test_encode_no_padding -v` | `enc = encode_cursor(CursorPayload(id="a", sort_key="views", sort_value=1)); assert "=" not in enc`. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-errors-1 | SCHEMA §1, §5, U3/U4 §errors | Each catalogued problem helper returns a response with media type `application/problem+json` and a body that schema-validates as `ProblemDetails`. | fourier | `test_problem_content_type` / `test_problem_schema_validates` | `uv run pytest "api/tests/test_crud_lib_errors.py::test_problem_content_type" -v` | Parametrized over the 20-row SCHEMA §5 catalog: `getattr(errors, name)().media_type == "application/problem+json"` and `ProblemDetails.model_validate(_body(...))`. |
...
| U-errors-2 | SCHEMA §1, §5 | ~~`problem()` rejects a `type` not in the catalogued set; raises `ValueError("unknown problem type")` at construction.~~ | fourier | — | — | **DEFERRED — NOT IMPLEMENTED.** `errors.problem(type_, ...)` accepts any `type_` string and performs no catalog-membership check (the closed set is enforced by exposing only the per-row `partial`-bound helpers, not by rejecting at the builder). No `ValueError` is raised and no test covers this. Struck per the no-phantom-citation rule. |
...
| U-errors-3 | SCHEMA §5, U3/U4 §errors | Every catalogued helper emits a `type` URI in `urn:contract:<kebab-name>` form (and the matching status). | fourier | `test_status_codes_match_catalog` | `uv run pytest "api/tests/test_crud_lib_errors.py::test_status_codes_match_catalog" -v` | Parametrized over the 20-row catalog of `(name, status, type)`; each asserts `getattr(errors, name)().status_code == status` and `_body(...)["type"] == "urn:contract:<kebab>"`. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-etag-1 | SCHEMA §1, U3/U4 §etag | `compute_etag(doc)` returns a value that **changes** when a field (e.g. `title`) changes. | fourier | `test_etag_differs_on_field_change` | `uv run pytest api/tests/test_crud_lib_etag.py::test_etag_differs_on_field_change -v` | `base = {"title":"a","visibility":"public"}; assert compute_etag(base) != compute_etag({**base, "title":"b"})`. |
...
| U-etag-2 | SCHEMA §1, U3/U4 §etag | `compute_etag(doc)` is **stable** across repeated calls on the same document content (and field-order independent — see U-etag-3 / `test_etag_field_order_independent`). | fourier | `test_etag_stable_for_same_doc` | `uv run pytest api/tests/test_crud_lib_etag.py::test_etag_stable_for_same_doc -v` | `for _ in range(100): doc = _rand_doc(); assert compute_etag(doc) == compute_etag(dict(doc))`. |
...
| U-etag-3 | SCHEMA §1, U3/U4 §etag | `compute_etag` output is a quoted 64-hex-char strong sha256 ETag (`"…"`, length 66, no `W/` weak prefix) per RFC 9110. | fourier | `test_etag_full_sha256_hex_shape` | `uv run pytest api/tests/test_crud_lib_etag.py::test_etag_full_sha256_hex_shape -v` | `e = compute_etag({"title":"a"}); assert e[0] == '"' and e[-1] == '"' and len(e) == 66; assert not e.startswith("W/")`. |
...
| U-etag-4 | SCHEMA §1, U3/U4 §etag | `require_if_match(request, etag)` raises 428 `urn:contract:precondition-required` when the `If-Match` header is absent (mutating method). | fourier | `test_require_if_match_missing_428` | `uv run pytest api/tests/test_crud_lib_etag.py::test_require_if_match_missing_428 -v` | `with pytest.raises(HTTPException) as exc: asyncio.run(require_if_match(_StubRequest(None), '"abc"')); assert exc.value.status_code == 428 and "urn:contract:precondition-required" in exc.value.detail`. |
...
| U-etag-5 | SCHEMA §1, U3/U4 §etag | `require_if_match` raises 412 `urn:contract:etag-mismatch` when `If-Match` is present but does not equal current ETag. | fourier | `test_require_if_match_mismatch_412` | `uv run pytest api/tests/test_crud_lib_etag.py::test_require_if_match_mismatch_412 -v` | `with pytest.raises(HTTPException) as exc: asyncio.run(require_if_match(_StubRequest('"other"'), '"abc"')); assert exc.value.status_code == 412 and "urn:contract:etag-mismatch" in exc.value.detail`. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-idem-1 | SCHEMA §1, U3/U4 §idem | `replay_or_record` for a previously-seen `(key, body_hash)` returns the **stored** response byte-verbatim; the handler runs only once. (Requires live Mongo; skips otherwise.) | fourier | `test_replay_returns_stored` | `uv run pytest api/tests/test_crud_lib_idempotency.py::test_replay_returns_stored -v` | Two `replay_or_record` calls with the same key `k2` + body; `len(calls) == 1` (handler ran once) and `bytes(r1.body) == bytes(r2.body)` (replay byte-equal). |
...
| U-idem-2 | SCHEMA §1, U3/U4 §idem | Same key, different body → 409 `urn:contract:idempotency-replay-conflict`. (Requires live Mongo; skips otherwise.) | fourier | `test_replay_different_body_409` | `uv run pytest api/tests/test_crud_lib_idempotency.py::test_replay_different_body_409 -v` | Record key `k3` with body `{"x":1}`; a second `replay_or_record` with key `k3` + body `{"x":2}` returns `status_code == 409` and body `type == "urn:contract:idempotency-replay-conflict"`. |
...
| U-idem-3 | SCHEMA §1, U3/U4 §idem | Idempotency-Key TTL is 24h, enforced by a Mongo TTL index on `created_at` with `expireAfterSeconds == 86400`. (Re-pointed: expiry is enforced by Mongo's TTL monitor; the realised unit test asserts the index exists rather than freezing time. Requires live Mongo; skips otherwise.) | fourier | `test_ttl_index_created` | `uv run pytest api/tests/test_crud_lib_idempotency.py::test_ttl_index_created -v` | After `ensure_indexes()`, exactly one index has `expireAfterSeconds is not None`; its `key == [("created_at", 1)]` and `expireAfterSeconds == 86400`. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-soft-1 | §5, U3/U4 §soft-delete | `soft_delete(coll, slug, owner_slug=...)` sets `deleted_at = <now>` for the owner and does **not** remove the document. (Requires live Mongo; skips otherwise.) | fourier | `test_soft_delete_sets_field` | `uv run pytest api/tests/test_crud_lib_softdelete.py::test_soft_delete_sets_field -v` | Insert `{slug:"a-b-c-d", owner_slug:"u", deleted_at:None}`; `ok = await soft_delete(coll, "a-b-c-d", owner_slug="u")`; `assert ok and find_one({slug}).deleted_at is not None` (document still present). |
...
| U-soft-2 | §5, U3/U4 §soft-delete | `not_deleted_filter()` returns the query predicate `{deleted_at: None}`; `with_not_deleted(q)` merges it without mutating the input. | fourier | `test_not_deleted_filter_value` | `uv run pytest api/tests/test_crud_lib_softdelete.py::test_not_deleted_filter_value -v` | `assert not_deleted_filter() == {"deleted_at": None}` (companion `test_with_not_deleted_does_not_mutate` confirms the live-row filter merges non-destructively). |
...
| U-soft-3 | §5, U3/U4 §soft-delete | ~~`include_deleted_filter()` returns `{}`; seeded with both rows, returns both.~~ | fourier | — | — | **DEFERRED — NOT IMPLEMENTED.** `api/lib/crud/softdelete.py` exposes no `include_deleted_filter` (the "include all" case is the absence of a filter, `{}`, applied at the call site); no such symbol and no test exist. The realised soft-delete surface instead adds a `restore` state machine, covered by `test_restore_within_grace_returns_restored` / `test_restore_past_grace_returns_expired` / `test_restore_not_found_and_idempotent_noop` (no §U row was allocated for restore). Struck per the no-phantom-citation rule. |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-cron-1 | §8, U3/U4 §cron | The recency prune is bounded by the compound index `(pinned, last_accessed_at)` that `cron_prune` targets. (Re-pointed: the realised helper is `pinned_cron.cron_prune` over a `(pinned, last_accessed_at)` index, not a `build_prune_query` / `deleted_at_1` plan. Requires live Mongo; skips otherwise.) | fourier | `test_cron_prune_uses_compound_index` | `uv run pytest api/tests/test_crud_lib_pinned_cron.py::test_cron_prune_uses_compound_index -v` | `create_index([("pinned",1),("last_accessed_at",1)])`; `assert any(idx["key"] == [("pinned",1),("last_accessed_at",1)] for idx in index_information().values())`. |
...
| U-cron-2 | §8, U3/U4 §cron | `cron_prune` source contains no `$nin` operator (the unbounded-scan anti-pattern). | fourier | `test_no_nin_in_query` | `uv run pytest api/tests/test_crud_lib_pinned_cron.py::test_no_nin_in_query -v` | `src = inspect.getsource(pinned_cron.cron_prune); assert "$nin" not in src`. Exit 0; 1 test passed. |
...
| U-cron-3 | §8, U3/U4 §cron | `cron_prune(coll, cutoff)` deletes only unpinned rows older than `cutoff`, leaves pinned and fresh rows intact, and is idempotent on re-run. (Re-pointed: the realised helper is `cron_prune`, not `prune_expired`. Requires live Mongo; skips otherwise.) | fourier | `test_cron_prune_deletes_unpinned_old_and_skips_pinned` | `uv run pytest api/tests/test_crud_lib_pinned_cron.py::test_cron_prune_deletes_unpinned_old_and_skips_pinned -v` | Seed 3 unpinned-old + 2 pinned-old + 2 unpinned-new; first prune deletes 3, leaves 4; a second prune deletes 0 (idempotent). |
...
| Assertion ID | Contract Section | Assertion | Repo | Test name | Run command | Expected output / fixture |
|---|---|---|---|---|---|---|
| U-meta-1 | U3/U4 §close | ~~The utility module's public surface matches the U3 spec — every exported symbol has at least one §U row, asserted by `test_utility_surface_complete_coverage`.~~ | fourier | — | — | **DEFERRED — NO TEST.** No surface-coverage test file (nor any `test_utility_surface_complete_coverage`) exists under `api/tests/`; the surface is declared in `api/lib/crud/__init__.py::__all__` but no test pins it against an `EXPECTED_SURFACE` set. Struck per the no-phantom-citation rule; revive when a surface-pinning test is added. |

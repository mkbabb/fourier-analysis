...
C inherits all 13 of tranche A's invariants (`docs/tranches/A/A.md §3`) and all 11 of tranche B's invariants 14–24 (`docs/tranches/B/B.md §2`) unchanged. It adds three. (**Numbering note — reconciled at fourier-F.W6 (C9):** these three restart at 18/19/20 and therefore overlap B-Inv 18/19/20; the canonical ledger `docs/tranches/INVARIANTS.md` records them as `C-Inv 18′ / 19′ / 20′` and binds them by *name* — delete-coupling / TLS-credentials / slug-convergence — never by bare integer. The single-replica clause inside #19 is the topology reading of `inv-19` cited in the infra precepts.)

...
| W1 — *Webhook CI/CD + secret extraction* | α | 2 parallel | webhook receiver deployed; `deploy.sh` retired (deletion proof, not commented out); the health-check port bug (curls 8091 vs live 8100, per `CA5`) is moot post-retirement; secrets out of `docker-compose*.yml`; commit-to-deploy chain recorded; rollback verified by an intentional bad commit | provisional |
...
| W4 — *Slug-identity completeness + B-residual discharge* | γ (+ conditional δ) | 2-3 parallel | the `snapshot_hash` DTO band renamed to `slug` end-to-end; the `as unknown as` cast removed; `FlaggedListResponse` reconciled to the cursor envelope; invariant-20 greps return zero; the 15 conformance skeletons filled-or-retired honestly (a skip-stub the matrix cites is either implemented or struck from the matrix — no aspirational citation); the e2e axe-keystone settle-wait landed; `vue-tsc -b --force` + `npm run build` green. **Conditional δ**: iff value.js has published `sampleToSVGPath` by W4 dispatch, `easings.ts`'s internal sampler swaps onto it (the internal dup collapses); else the consume holds as a named residual. **Independent of W1–W3/W5 at file bounds — may run parallel with the infra waves.** | provisional |
| W5 — *Image-blob migration* | β | 2-3 parallel | `image_storage.py:104` no longer writes `Binary(content)` (deletion proof); the chosen backend stores primary + thumbnail blobs with `storage_uri` recorded on the `images` doc; migration script + verification harness (count parity, seed-spot-check, post-condition) per `R-lifecycle-spec.md §5.1`; `GET /api/images/{slug}/blob` serves from the new backend. (Note: the `storage_budget_gb` config + eviction were already retired at B.W3 — `CA5`; this wave's gate is the inline-write relocation, not the config retirement.) | provisional |
...
- `scripts/deploy.sh` does not exist; webhook deploy proven by a recorded commit-to-deploy chain.
...
- The 15 `api/tests/conformance/*.py` skeletons are each implemented (passing) or struck from `CONFORMANCE-MATRIX.md` — no skip-stub remains cited as evidence.
- `api/services/image_storage.py` source contains no `Binary(content)` write (the W5 deletion proof); the chosen backend's enumeration query returns a bounded total recorded by the migration verification; primary + thumbnail both relocated.
...
The W5 image-blob migration may require a brittleness window — a span where reads are dual-pathed (Mongo `blob` field AND new backend `storage_uri`) until the new backend is verified complete. Whether the chosen backend admits an atomic cutover is a research question (Wα-R1); `CA5` ranks **filesystem+nginx** first precisely because it admits an atomic per-document cutover, removing the window.

```yaml
breaking_changes_during_wave: maybe (W5)
suspended_gates:
  - GET /api/images/{slug}/blob during the migration cutover
restoration_wave: W5 (same wave — the migration completes within it)

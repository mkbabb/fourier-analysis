...
|---|---|---|---|
| W0 — Open + audit intake + cheapest γ | planned | — | F close re-confirmed (survives falsification per GA3); 6-lane G-audit (GA1-GA6 + SYNTHESIS at `docs/audits/runs/2026-05-29-G-audit/`) committed as binding baseline; the 2 one-line γ items (misleading `rate_limiter.py:143` comment + `compute_cache.py:105` `datetime.utcnow`→`now(tz=UTC)`) land as cheapest excision |
| Wα — Research-light (2 lanes) | planned | — | α: confirm the CF Pages frontend-deploy invocation (`deploy/cf/pages-deploy.sh`) + that it can run from a tracked fourier script. β: confirm nginx `real_ip` resolves the true client through Apache's XFF (the 2-hop chain) |
| Wχ — Challenge (3 probes) | planned | — | P1 inv-26 single-source decision (codegen-canonical vs hand-types-canonical); P2 T2 re-tightened per-client read budget value; P3 δ self-host blast radius (fonts + KaTeX CSS) |

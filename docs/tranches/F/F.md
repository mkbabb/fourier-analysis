...
- **F-γ receipts** *(SPLIT per Wχ-P1)*: `receipts/F-W3a.json` (host-ops single-window: staged hooks.json + speedtest teardown + cron/dangling capture) AND `receipts/F-W3b.json` (operator-gated: gh re-auth + 5-URL flip + hook tests + dispatcher `rm`). W3b gates on `gh auth login`; the close is HONEST if W3b is named-residual-with-operator-owner at F close (the dispatcher stays until the 5 URLs flip — deleting it earlier 404s webhooks).

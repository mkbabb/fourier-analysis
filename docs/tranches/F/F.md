...
- **S1/S2** survey committed-credential flags (floridify `pk_*`; speedtest `cfat_*`) — **WITHDRAWN** (maintainer determination 2026-05-28: nothing needs rotating — Clerk `pk_*` publishable by design; speedtest `cfat_*` non-sensitive). Recorded resolved in `SURVEY-FINDINGS.md §3`; no remediation, in or out of F scope.

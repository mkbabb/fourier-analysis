...
## §18 — Ordering λ′ (I — AUTHORED 2026-06-01, awaits user "Begin")

**fourier-I AUTHORED 2026-06-01** (development only; not executed) — `docs/tranches/I/I.md`, substrate the 6-agent modern-web constellation audit read through the Chrome `modern-web-guidance` v0.0.170 lens (12 categories): the approved plan `docs/audits/runs/2026-06-01-modern-web/modern-web-tranche-plan.md` + the 6 per-repo reports (`{fourier,value.js-color,glass-ui,keyframes,words,speedtest}.md`) + the constellation style-audit (`docs/audits/runs/2026-06-01-constellation-ui/style-audit.md`) + the drafted `glass-ui-root-rounding.patch`. I is the **constellation-modernization tranche (Chrome modern-web-guidance adoption, glass-ui-rooted)** — the successor to H's hygiene + green-means-green sweep.

**Why I succeeds the §17 "no fourier tranche open / A–H all closed" posture**: H made "green" TRUE (fourier CI actually green for the first time — run `26773946417`, HEAD `8058c44`; reliable across 4 consecutive runs) and brought the constellation's CI from red toward green or honestly booked. With the signal now honest and the surfaces strong, the forward question changes from *is it green?* to *is it modern?* The six audits converge on one verdict: these are **already-strong, heavily-audited surfaces**, but the modern-web drift is **concentrated, repeated, and high-value** — it clusters into ~9 cross-repo themes the platform only recently made native (overlay positioning still 100% JS; no `content-visibility` on the heaviest surfaces; JS scroll-listeners where CSS scroll-driven animation now runs on the compositor; no View Transitions on any SPA; combined-`transform` hover without the individual-property identity; image-hygiene gaps; an untouched forms/native-`<select>` category; a thin CSP/analytics tail). That is a modernization opportunity, not a residual.

**The leverage principle (the spine of the sequencing)**: glass-ui is the shared design system — an adoption that lands in `glass-ui/src` propagates to every consumer (fourier, value.js, words, speedtest, keyframes-demo, bbnf-buddy) at once. Therefore **glass-ui-rooted waves rank highest**, and within them the *substrate* fixes (overlay positioning, transform discipline) rank above leaf fixes. The two audits converge at one seam: the style-audit's gap-#3 *hover/press-scale* (~69 sites, largest demand) and the modern-web glass-ui-D3 *individual-transform-properties* are the **same motion-layer rewrite** — α (the highest-ROI first move) closes the largest style-audit demand, satisfies the modern-web guide, AND unblocks the β anchor-positioning keystone.

**The 9-wave shape** (α–ι, by leverage × dependency): **α** glass-ui transform-identity + scale utility (**FOUNDATION** — base `scale:1`/`translate:0` identity + `.hover-scale`/`.active-scale`/`.hover-cartoon` using individual transform props; closes ~69-site style-audit demand + unblocks anchor positioning) → **β** native overlay substrate (**KEYSTONE** — popover API + anchor positioning + top layer; named by 4 of 6 reports; deletes the `useDialogOverlayGuards` bug-class) → **γ** content-visibility (**HIGHEST-ROI PERF** — #1 in 3 reports; near-zero risk) → **δ** scroll-driven CSS (retires JS scroll listeners onto the compositor) → **ε** View Transitions (retires 3 FLIP engines) → **ζ** `@starting-style` top-layer (enter/exit from `--spring-*`) → **η** forms/native-`<select>` (`base-select` PE) → **θ** image/asset/privacy (fourier avatar+figures + sibling fonts/images) → **ι** CSP/analytics/INP-tail + **passkeys BOOKED-only** (no app owns a credential surface).

**Two new invariants**: **inv-29** (progressive-enhancement-floor — a modern-platform-API adoption ships BEHIND an `@supports`/feature gate with the prior path retained as the floor; Safari/Firefox parity; NO rip-out — the modern path is the enhancement, not a non-supporting-engine-breaking replacement) + **inv-30** (platform-over-library — prefer a native capability where the platform now suffices; the library/JS path demotes to the `@supports`-false floor or retires only when the platform is universally sufficient; the modern-web analog of inv-15). Both continue the clean fresh-integer sequence (G 25/26, H 27/28, I 29/30) and are **namespace-partitioned** from the glass-ui precepts' `invariant 29`/`invariant 30` exactly as `H-Inv 28` is partitioned from the precepts' `invariant 28` (`INVARIANTS.md §2.11 + §3`).

**The mid-flight BOOK-ALL posture (H.ε inherited)**: the constellation is mid-flight (2026-06-01 recon: keyframes +19, glass-ui +112 incl. the just-committed root-rounding `b6d6cf4`, words +11, speedtest +563 unpushed). So fourier-I is **fourier-rooted with an authorized cross-repo sweep under inv-16′** that is **authored-now / executed-on-clean-checkout** — the glass-ui-rooted foundations (α/β/ζ/η) + the sibling leaf adoptions are NAMED, file-verified `ADOPTION-ASKS` entries, per-repo-green-CI-gated, NOT swept against a dirty tree (inv-16 held). Only the **fourier-local** waves execute this session against fourier's clean tree.

**Critical path at λ′**:
- **fourier (executable spine, fourier-local)**: **I.W1 (γ content-visibility on the paper window sections + the off-screen epicycle canvas + inactive views)** is fourier's first move — its #1 finding, S-effort, independent of α/β (runs in parallel from W0); then δ floating-TOC (W4), ε `/w/`↔`/v/` (W5), θ avatar+figures (W7). Each gated on a measured LCP/INP delta + a green fourier CI run id (inv-27).
- **glass-ui-rooted critical path (authored-now / executed-on-clean-checkout, inv-16′)**: **α (FOUNDATION) → β (KEYSTONE) → ζ/η**, glass-ui-rooted first; α blocks β (a hover-only combined-`transform` breaks `anchor()`); β unblocks the leaf overlay-deletions in θ. δ/ε compose with α/β.
- **sibling leaves (inv-16′ asks, mid-flight book-all)**: value.js (fonts self-host + 5 comboboxes + ghost-pane); words (`CarouselSlide` + `<search>` semantics); speedtest (content-visibility tables/charts + LCP `<img>` + `fetchLater()`); keyframes (previews + the narrowed-remit DOM-API→native per inv-30) — each booked with its owner, executed on its own clean checkout, per-repo-green-CI-gated.
- **value.js-J** (E5/E6/E7) remains on its own driver; the C1/C5/C6 carries STAY OUT (owning-repo-tranche-gated, not CI-swept).

I is single-repo-rooted with an authorized cross-repo sweep, inv-16′-bounded, and (like H) **AUTHORED only — it awaits the user's "Begin"** before Phase 0 (W0 → Wα research-light → Wχ challenge) opens.

End of CANONICAL-ORDERING §18 — ordering λ′.

## §19 — Ordering μ′ (J — AUTHORED 2026-06-02, awaits user "Begin")

**fourier-J AUTHORED 2026-06-02** (development only; not executed) — `docs/tranches/J/J.md`, the CORE design docs `docs/tranches/J/design/{J.W1-crud-remix.md, J.W1c-publish-visibility.md}`, the canonical diff-shape doc `docs/tranches/J/design/J-diff-shape.md` (J.W1), substrate the constellation WAVE-D audit seed `glass-ui:docs/constellation/next/audit/A3-fourier-valuejs.md` (repo-qualified per A6-1) + the 6-lane deep audit `docs/audits/runs/2026-06-02-J-deep-audit/` (A1–A6 + the SYNTHESIS forward path). J is the **visualization REMIX tranche (CRUD / atom-diff provenance + publish-visibility CORE)** — the successor to I (the constellation-modernization tranche, ordering λ′). μ′ is the J ordering letter (λ → μ), the correct next letter in the fourier-rooted Greek chain.

**Why J succeeds the §18 "A–I all CLOSED GREEN" posture**: I made the platform *modern* but never touched the data model. The audit found the asymmetry file:line — fourier's viz-server has a remix READ side (`cursors.py:21` sorts `most-forked` by a phantom `fork_count` that is never written and is not on the `Visualization` model) and **no remix WRITE side** (no fork endpoint, no version collection, no provenance walk, no atom-diff); value.js's palette-server ships the whole fork+version+provenance machinery but also lacks atom-diff. So the close is symmetric: **fourier INHERITS the fork/version/provenance substrate value.js proved; both repos GAIN the atom-diff layer + the publish/visibility facet.** The CORE (CRUD/REMIX atom-diff, single-parent LINEAR, flat content-addressable atom bag, per-atom set-difference, no DAG/merge/rebase/CRDT — the KISS line) is genuinely idiomatic to the existing `lib/crud` compose-don't-frame discipline; the remix API is authored agent-legibly so the future WebMCP tool surface (booked-not-built; named successor **K**, hard external gate Chromium 146 stable) stays a thin wrapper.

**The deep-audit fold (this run) refined W0/W1 + sharpened W2** (the consolidated forward path is `docs/audits/runs/2026-06-02-J-deep-audit/J-deep-audit.md §4 P0/P1 + §7 steps 1–9`):
- **W0 (evidence-chain + chronic ledger)**: repo-qualify the A3 seed to `glass-ui:…` (it 404'd in both repos); write THIS §19 / ordering μ′ (it was unwritten — J's close criterion cited it); seed the three discrete inv-16′ ADOPTION-ASKS rows the J plan asserted-present but had not ledgered (`valuejs-J-atomdiff`, `valuejs-J-publish`, `glass-ui-P5-inner-rounding` — `docs/constellation/ADOPTION-ASKS.md §7`); record the chronic ledger with terminal verdicts (KILL C1's silently-dropped colour-lift consume-ask; KILL VAL-9, BOOK VAL-1 with a hard kill-date, force a terminal verdict on the value.js 6-tranche glass-ui carry — all value.js-J.W0, fourier disclaims correctly; zero perpetual punts).
- **W1 (CORE design, both repos bind)**: pin ONE canonical repo-neutral `/diff` shape doc (`J-diff-shape.md` — the claimed parity was NOT isomorphic: op vocabulary `modified`/`changed`, field `atom_diff`/`ops`, value.js's 4 extra body fields would FAIL the close-gate probe) + ONE pattern name (`atomdiff`, reconciling a three-way module-name split); **wire-in and symmetrize the orphaned publish/visibility CORE facet** (`J.W1c-publish-visibility.md` — unreferenced by the plan, no value.js peer; it discharges the dead `visibility_illegal_transition` guard and wires value.js's missing public-view filter); drop the vestigial 6-wave-spine mapping (re-present J as CORE + consumer-backed leaf wins + close); rule on the orphaned WAVE-C design-polish thread.
- **W2 (sharpened — the P0 + contract gaps)**: re-express the remix's `withTransaction` (which has ZERO codebase precedent and which standalone Mongo cannot run → would skip-or-fail under inv-27) as an **ordered idempotent content-addressed write sequence** (content-addressed `_id=set_hash`; slug-unique child; conditional last `fork_count $inc`; delete-race guard) — a design decision resolved as a W1 amendment BEFORE W2 opens; make `canonical_digest` (one serializer, three projections) the first W2 code move; then palette_slug FK validation, the two named provenance walks, the bounded `/versions` chain (no cursor — it collides with the 64-hex `set_hash` `_id`), the hand-typed Pydantic response twins (inv-26), value.js idempotency pulled forward + the conformance dir.

**Critical path at μ′**:
- **fourier (executable spine, fourier-local)**: J.W1 CORE design → W2 CORE impl (the value) → the consumer-backed leaf wins (W3 `scheduler.yield()` — the highest remaining INP lever I named but never ran; W4 gallery-grid `content-visibility` — the unapplied `.deferred-section` consumer; W5 the e2e/axe CI EVIDENCE wave — the inv-27 green claim I could NOT make in-session; W6 the CSP/`fetchLater` tail) → W7 close. Each leaf gated on a measured CWV delta + a green fourier CI run id (inv-27).
- **cross-repo arm (inv-16′, authored-now / executed-on-clean-checkout)**: value.js-J adopts the same atom-diff PATTERN over `PaletteColor[]` atoms (`forkPalette`→`remixPalette`, `PaletteVersion.atom_diff`, `GET /:slug/diff?from=`) + the symmetric publish/visibility verb pair + the missing public-view filter — the three `ADOPTION-ASKS.md §7` rows, each its own commit, per-repo-green-CI-gated. The constellation was mid-flight at authoring (H.ε BOOK-ALL posture inherited).
- **value.js-J** carries its own chronic re-gates (VAL-9 KILL / VAL-1 kill-date / the 6-tranche glass-ui carry terminal verdict) on its own W0; the C1/C5/C6 carries STAY OUT (owning-repo-tranche-gated).
- **named-forward (not a J wave)**: the WebMCP tool surface → **K** (Chromium 146 stable); passkeys (no credential surface).

**No J close cites μ′ as evidence until the W7 close lands** (inv-27 — green-means-green / evidence discipline): μ′ is written here at the W0 obligation (J's completion criterion requires "CANONICAL-ORDERING → ordering μ′"), but a "J CLOSED" claim is honest only once `docs/tranches/J/FINAL.md` lands with every commit + the binding green CI run ids + the `/diff` shape-parity verdict against `J-diff-shape.md` + the publish-envelope-parity round-trip + the inv-16′ value.js-J ledger + the chronic-resolution block. Until then μ′ records an AUTHORED tranche, exactly as κ′ first recorded H and λ′ first recorded I.

J is single-repo-rooted with an authorized cross-repo sweep, inv-16′-bounded, and (like H and I) **AUTHORED only — it awaits the user's "Begin"** before Phase 0 (W0 audit intake → W1 CORE design) opens. The deep-audit run dir is `docs/audits/runs/2026-06-02-J-deep-audit/`.

End of CANONICAL-ORDERING §19 — ordering μ′.

## §20 — Ordering ν′ (K-deploy — AUTHORED 2026-06-04, awaits user "Begin")

**fourier-K-deploy AUTHORED 2026-06-04** (development only; not executed) — substrate the 6-dimension J post-implementation audit `docs/audits/runs/2026-06-04-J-postimpl-audit/` (A1 plan/waves · A2 W2 CORE code · A3 deploy chronic · A4 e2e/CI chronic · A5 prompt/precept coverage · A6 chronic/deferred/no-legacy + the SYNTHESIS forward path). K-deploy is the **constellation-infra + cross-repo chronic tranche** (the deploy backlog/health-gate + the e2e/glass-dock inv-27 gate as its CORE) — the successor to J (the visualization-REMIX tranche, ordering μ′). ν′ is the K-deploy ordering letter (μ → ν), the correct next letter in the fourier-rooted Greek chain (the chain reads κ′=H → λ′=I → μ′=J → **ν′=K-deploy**).

**Why K-deploy succeeds J — without J closing**: J's CORE (atom-diff/remix/publish) is real, idiomatic, and 266/266 green locally, but J CANNOT honestly close — its CI is red (inv-27 unmet, no green run id over `9d7c387`), W5–W8 are all `planned`, the CORE has zero frontend consumer (a binary inv-15 breach — `grep api.ts` for the 7 endpoints = 0 hits), and the deploy-of-record never landed (host stuck at `f2fe447`, 29 commits behind — verified `git log f2fe447..9d7c387` = 29). The audit's shape verdict (SYNTHESIS §0) is therefore **two interleaved tranches**: J STAYS OPEN to complete its data-model arc (W2-fix integrity + W2-transpose elegance-delete + W5/W6 wire-the-UI + the e2e green), and the constellation-infra + cross-repo chronics — which are F/H-lineage, NOT J-local data-model work — earn their OWN named tranche rather than a quiet J residual. Dropping a months-class deploy chronic into a data-model close is the anti-pattern the directive forbids.

**The CORE = the two highest-stakes chronics**: (1) the **29-commit babb.dev deploy backlog + the health-gate** (G/H/I/J never deployed; the F "2-month silent webhook" anti-pattern, structurally — the API arm has no green-CI gate and a rollback reaches no constellation-watched sink) and (2) the **e2e/glass-dock inv-27 gate** (the `glass-ui-dock-vt-name` chronic — fourier's e2e red since before this session). This is the tranche the directive's "fold ALL chronic items into a NEW tranche" demands for the infra/cross-repo class.

**The load-bearing interleaving (named explicitly so no future session reads "J open, K open" as a contradiction)**: J's W6 inv-27 close gate has a **HARD UPSTREAM dependency** on K-deploy's W4 — the glass-ui-3.2.0 dock-VT `useId()` fix (`ADOPTION-ASKS.md §8` / the `glass-ui-dock-vt-name` row). So although J is the "older" tranche, **K-deploy executes the e2e unblock that J's inv-27 close gate consumes.** The two run **interleaved, not strictly sequential** — J cannot reach its green-run close gate until K-deploy (or the glass-ui maintainer it asks) lands the dock fix. This is the dependency-direction insight the SYNTHESIS §0 calls "circular-looking but real": the bridge (a kill-dated exact-string console-filter at J.W6) makes the suite green NOW while the real fix is owned and 3.2.0-gated; the *honest* J close still waits on the upstream.

**The deploy root-cause (A3-confirmed; the prior hypotheses RULED OUT)**: the gate failure root-causes to **backend lifespan startup-blocking** (`api/main.py:42` `await connect_db()` inside lifespan; `database.py:24-48` raises on timeout then builds ~30 indexes incl. the new J fork indexes *before* `/api/health` answers) **+ the 60s gate being too tight for a 29-commit cold WORKERS=1 start with no container HEALTHCHECK**. A3 RULED OUT the **J backend** (empty prod DB → no-op migration runs post-gate) and the **inv-22 `/→404` half** (nginx-direct, not the backend) as the cause — correcting the session's own "or the frontend" hypothesis. The idiomatic fix is non-blocking-after-serving index creation OR a real container `HEALTHCHECK` + `depends_on: condition: service_healthy`, NOT a blind longer poll.

**The named-forward correction (chain honesty)**: the booked WebMCP feature graduation (Chromium-146-stable-gated) that `J.md §10` named "K" is **RENAMED L-webmcp** — so the infra chronic (K-deploy, ν′) and the feature graduation (L-webmcp, further-out) are never conflated. The infra tranche takes the K letter because it is the next-executed successor; the feature booking slides one letter out.

**Critical path at ν′**:
- **K-deploy (executable spine, fourier-local + host-coupled)**: K.W0 (open + write THIS §20 / ordering ν′ + confirm the `glass-ui-dock-vt-name` row seeded + name the single human dependency — the host-only read-only GitHub PAT for the inv-28 API-arm gate, operator-owned) → K.W1 (LAND the 29-commit backlog with an OBSERVED-green HEALTHCHECK gate, not a blind poll) → K.W2 (BUILD the inv-28 API-arm green-CI gate — `green_ci_gate()` querying `commits/<sha>/status` BEFORE `git reset --hard`; **fails closed** without the PAT, the correct safe state) → K.W3 (kill the silent-rollback chronic — the three terminal states POST to a constellation-watched sink; backport to `deploy/templates/deploy-hook.sh`) → K.W4 (the glass-ui-3.2.0 adoption wave — the dock-VT `useId()` fix + P5-inner-rounding + a11y `inert` + `asideSide` + `useTextHighlight`; **UNBLOCKS J.W6**) → K.W5 (operator entrypoint adoption) → K.W6 (the value.js-J inv-16′ peers — `valuejs-J-atomdiff`/`valuejs-J-publish`, cohort) → K.W7 (`dispatch.sh` retirement, gated on all-four-migrate) → K.W8 (close).
- **J (interleaved, the consumer of K.W4)**: J's W6 inv-27 EVIDENCE wave + W8 close are *downstream* of K-deploy's e2e unblock; J's W2-fix/W2-transpose/W5 run on J's own driver in the meantime (they do not block on K).
- **cross-repo arm (inv-16′, authored-now / executed-on-clean-checkout)**: the `glass-ui-dock-vt-name` row is glass-ui-maintainer-owned (inv-16; fourier holds no lever — no `!important` workaround, NO-LEGACY per the user); fourier's only act is the K.W4 3.2.0 adoption once the upstream fix ships, per-repo-green-CI-gated.
- **value.js-J** remains on its own driver (its VAL-9 KILL / VAL-1 kill-date / 6-tranche glass-ui-carry terminal verdicts on its own W0); the C1/C5/C6 carries STAY OUT (owning-repo-tranche-gated).
- **named-forward (not a K wave)**: the WebMCP tool surface → **L-webmcp** (Chromium 146 stable); passkeys (no credential surface).

**No K-deploy close cites ν′ as evidence until the close lands** (inv-27/inv-28 — green-means-green / verified-deploy-of-record): ν′ is written here at the K.W0 obligation (K-deploy's completion criterion requires "CANONICAL-ORDERING → ordering ν′"), but a "K-deploy CLOSED" claim is honest only once `FINAL.md` cites **observed-green prod** at `9d7c387+` (K.W1) + the **inv-28 API-arm green-CI gate that fails-closed-without-PAT** (K.W2) + a **rollback that pages a constellation-watched sink** (K.W3 — the months-old silent-rollback chronic, structurally killed, never to whisper a third time) + the dock-VT fix landed → J.W6 unblocked. Until then ν′ records an AUTHORED tranche, exactly as κ′ first recorded H, λ′ first recorded I, and μ′ first recorded J.

K-deploy is single-repo-rooted (host-coupled) with an authorized cross-repo sweep, inv-16′-bounded, and (like H, I, and J) **AUTHORED only — it awaits the user's "Begin"** before Phase 0 (K.W0 open + ledger) opens. The J-postimpl-audit run dir is `docs/audits/runs/2026-06-04-J-postimpl-audit/`.

End of CANONICAL-ORDERING §20 — ordering ν′.

## §21 — Ordering ξ′ (M — AUTHORED 2026-06-16, awaits user "Begin")

**fourier-M AUTHORED 2026-06-16** (development only; not executed) — `docs/tranches/M/{M.md,PROGRESS.md}` + `design/{M-design-language.md,M-motion-architecture.md,M-bump-migration.md}`, substrate the 32-agent deep audit `docs/audits/runs/2026-06-16-M-deep-audit/` (8 plan/recap + 7 per-pane design + 7 design-language + 4 glass-ui + 6 animation auditors, batched three-at-a-time on Opus/Sonnet; two Opus synthesis digests `plan-synth.json` + `design-synth.json` merged by the core model; 303 findings / 32 critical / 82 chronic-flagged / 59 glass-ui asks; `findings-index.txt` index; `raw-findings.json` corpus; the three session-limit/socket re-runs `{A8,B1,B6}-*.md`). M is the **convergence-and-suffusion tranche** — the EXECUTION discharge of J + K-deploy on a re-grounded glass-ui-4.0.0 world, the design-language suffusion, and the unified mathematical motion architecture. ξ′ is the M ordering letter (ν → ξ), the correct next letter in the fourier-rooted Greek chain (κ′=H → λ′=I → μ′=J → ν′=K-deploy → **ξ′=M**).

**Why M succeeds — and ABSORBS — J (μ′) and K-deploy (ν′):** the 32-agent recap (A1) found the deepest pattern in the whole corpus is a PROCESS chronic — the recurring *"deeply audit / idiomatic gestalt path forward / fold ALL chronics / recap ALL prompts / NOT implementation"* meta-directive has been honoured at the PLANNING layer ~6 times while the two PHYSICAL chronics it keeps re-planning have never shipped: (1) the deploy-of-record never landed (host **44 commits behind** `f2fe447`, re-verified live — worse than the 29 `K.md` records; G/H/I/J never deployed; silent rollback for months) and (2) the inv-15 consumer gap (J's 7 CORE endpoints have ZERO `api.ts` caller; `gallery.publish` still PATCHes — a binary breach at HEAD). M **resolves this by construction**: it is not a seventh re-plan; it EXECUTES J's open remainder (the W2-fix/W2-transpose data-model + the W5/W6 wire-the-UI + the e2e green) and K-deploy's fourier-side spine (HEALTHCHECK + the inv-28 API-arm gate + the page-on-rollback sink), so **J + K-deploy + M close together at M's close** (their FINAL.md files cite M's commits). M's close-gate is PHYSICAL — observed-green prod (host caught up to HEAD) + a live UI consumer per endpoint + a single covering green inv-27 run + the design language demonstrably shipped — not a status claim.

**The re-grounding that dissolves the prior premise (the inv-32 motive):** every prior fourier audit — and the 11-day-stale `RUN-BOARD.md`, whose "BLOCKED root hinge E1 = glass-ui 3.3.0 on npm" long since cleared — assumed a glass-ui **3.2.0** world. Reality, re-verified live (`npm view`): glass-ui **4.0.0** (a MAJOR over fourier's `^3.1.0`), keyframes.js **4.2.0** (TWO majors over `^2.2.0`), value.js **0.12.0**. The K-deploy "one-line `^3.1.0→^3.2.0` caret bump" is dead at the root; M.W1 is a multi-major reconciliation with a real ~43-edit breaking surface (source-verified in `M-bump-migration.md`). The happy inversion: nearly every primitive the last three audits booked as a *future* glass-ui ask has ALREADY shipped (dock `useId`, `asideSide`, `dividers`, `.configurator-section-label`, `useTextHighlight` on `/motion-core`, `inert`, `cartoon-surface`, `useSpecularTracking`, the scroll-driven + View-Transition recipes), so the design/motion work collapses from "wait + hand-roll" to **"bump + consume + delete"** — net code DECREASES.

**Four new invariants (clean fresh integers 31–34 + an inv-29 amendment;** `INVARIANTS.md §1`, authored at M.W0): **inv-31** observability-floor (K-reserved, M.W4-authored — a deploy/rollback terminal state pages a watched sink, or the deploy path is NOT closed; the chain went silently-broken-for-months TWICE); **inv-32** version-currency (a pin must not trail a sibling's published major by >1, AND any "gated on a future sibling release" edge cites a dated `npm view` re-verification within the tranche's W0 — the structural fix for the I→J→K book-rot); **inv-33** design-system-single-source (one platform source per axis: control panes are `Configurator` instruments, glass surfaces pick their rung by depth band, all runtime colour derives from value.js over the icon-rainbow OKLCH ramp; strengthens inv-10, hardens inv-30); **inv-34** motion-proportion floor (zero rAF animation call-sites, the three-layer architecture, PRM-honoured at the engine layer, INP-safe, proportion-ceilinged). All namespace-partitioned from the glass-ui precept invariants 31/32/33 (`INVARIANTS.md §3`).

**Critical path at ξ′:**
- **fourier (executable spine, two arms joining at evidence)**: **M.W0** (charter + re-ground sweep + ledger/numbering hygiene) → **M.W1** (the multi-major bump KEYSTONE — greens the e2e at source, unblocks all five mandate axes) → then **Arm A** (infra, bump-independent: M.W2 HEALTHCHECK+`service_healthy`+index-off-serving-path · M.W3 fail-closed inv-28 API-arm gate · M.W4 page-the-sink+inv-31+land-a-green-SHA→host-catches-up) runs PARALLEL to **Arm B** (product, bump-gated: M.W5 glass depth ladder · M.W6 audacious typography · M.W7 OKLCH colour pops + math/grid chrome · M.W8 control-pane hierarchy [deps W5] · M.W9 unified motion CORE) → the arms join at **M.W10** (wire the inv-15 consumer gap + data-model transpose + scroll heroes — the J product close) → **M.W11** (EVIDENCE — one covering green inv-27 run + measured CWV/INP + axe) → **M.W12** (tail hygiene + cohort parity + book-with-kill-dates) → **M.W13** (close). Critical path: W0→W1→W9→W10→W11→W12→W13; Arm A parallel.
- **cross-repo arm (inv-16′)**: the glass-ui ADOPTION-ASKS (the tonal-accent recipe, the AtomDiff viewer, the ConfiguratorLayer trailing slot, the DockIconButton active prop, useCharStagger/SplitChars, canvas-anchored-overlay, the once-discipline scroll-reveal, the convergence-reveal preset, the glass-tier staleness gate) are booked-not-written — fourier consumes glass-ui, never writes it. The residual `dispatch.sh` retirement + the adoption asks 1–7 are re-verified-live + escalated at M.W12 (no 7th silent roll; fourier holds no lever, inv-16). The A-1/A-2/A-3 carries are STRUCK (SHIPPED at 4.0, source-confirmed).
- **value.js-J** peers are DONE-in-value.js-L; M owes only its OWN fourier-local Python `/diff` + publish parity probe. **L-webmcp** (Chromium-146-stable WebMCP graduation) stays the further-out successor, external-gated, re-affirmed at M close.

**No M close cites ξ′ as evidence until the close lands** (inv-27/28 — green-means-green / verified-deploy-of-record): ξ′ is written here at the M.W0 obligation (M's completion criterion requires "CANONICAL-ORDERING → ordering ξ′"), but an "M CLOSED" claim is honest only once `FINAL.md` (+ the J and K-deploy close ledgers M discharges) cites observed-green prod at HEAD + the green inv-27 run id + a live consumer per endpoint + the design language shipped. Until then ξ′ records an AUTHORED tranche, exactly as κ′ first recorded H, λ′ recorded I, μ′ recorded J, and ν′ recorded K-deploy.

M is single-repo-rooted (fourier-local + host-coupled) with an authorized cross-repo book-only arm, inv-16′-bounded, and (like H, I, J, K-deploy) **AUTHORED only — it awaits the user's "Begin"** before Phase 0 (M.W0) opens. The deep-audit run dir is `docs/audits/runs/2026-06-16-M-deep-audit/`.

**AMENDMENT 2026-06-16 (`M/M-AMENDMENT-latest-and-BB.md`; 3 user directives, supersede the version prose above):** (1) the landing IS the paper (the §4.1 hero grafts onto PaperView in place, no `/` route); (2) ALL packages to latest — the targets above are stale: keyframes is **4.3.0** (not 4.2.0), value.js **0.13.0** (not 0.12.0; `sampleColorRamp` shipped), + the framework majors **vite 8 / vue-tsc 3 / vue-router 5 / pinia 3 / lucide 1.0** + katex 0.17; (3) **glass-ui's tranche BB is the active upstream** — fourier's glass-ui target is **`^4.1.0` (BB's fold-all close cut), GATED on glass-ui BB publishing** (the first hard upstream edge M carries: `M.W1b ← glass-ui BB 4.1.0`; inv-32 re-verifies `npm view ≥ 4.1.0`). M's asks route to BB's `W-CROSSREPO-ASKS`; M consumes BB primitives; glass-ui BB executes under the glass-ui arm (inv-16 — fourier consumes, never writes it). The chain head is unchanged (ξ′=M); the bump keystone now splits W1a (ungated, published) → W1b (glass-ui, BB-gated).

**AMENDMENT 2026-06-17 (`M/M-AMENDMENT-critique-hardening.md`; the live-critique 32-agent audit):** M.W1 was EXECUTED this session (the all-latest bump + the breaking-surface migration, branch `m/w1-bump-migration`); the user critiqued the live bumped instance (8 findings). The audit (316 findings, 29 alpha) HARDENED M without re-planning it: **4 new invariants inv-35..38** (glass-fidelity / geometry-continuity / control-affordance / render-verified-design-gate — `INVARIANTS.md §1`) close the seam that nothing bound *rendered* fidelity. 6 of 8 critiques are fourier-owned (folded into M.W5/W7/W8 — incl. the headline reversal of the M.W1 `variant="underline"` → glass pill); 2 are glass-ui-owned (booked to glass-ui BB `W-CROSSREPO-ASKS`, `ADOPTION-ASKS §12`). **The entire design surface (W5–W9) + value.js 0.13 + the e2e-green close are W1b-gated on glass-ui BB publishing 4.1.0** — only W0/W1a/W2–W4 proceed now. The chain head is unchanged (ξ′=M).

End of CANONICAL-ORDERING §21 — ordering ξ′.

...
import { highlightFuzzy } from "@/components/paper/search/searchHelpers";
 ...
 *
 * `.g`'s `paper-search.vitest.ts` pins `fuzzyMatch`'s ordering contract. This
 ...
 */
describe("fuzzyMatch — `PSM-14`, the length penalty", () => {
    ...
    it("scores an exact mid-body word hit ABOVE zero (the drop threshold)", () => {
        ...
        const m = fuzzyMatch("fourier", BODY);
        expect(m).not.toBeNull();
        expect(m!.score).toBeGreaterThan(0);
    });

    it("scores short prefixes above zero — `\"fo\"` and `\"conv\"` were −31.6 and −36.4", () => {
        expect(fuzzyMatch("fo", BODY)!.score).toBeGreaterThan(0);
        expect(fuzzyMatch("conv", BODY)!.score).toBeGreaterThan(0);
    });

    it("still prefers the tighter field — the penalty is re-shaped, not removed", () => {
        const tight = fuzzyMatch("fourier", "fourier series")!.score;
        const loose = fuzzyMatch("fourier", BODY)!.score;
        expect(tight).toBeGreaterThan(loose);
    });

    it("never returns a negative score, at any text length", () => {
        const veryLong = BODY.repeat(6);
        expect(fuzzyMatch("f", veryLong)!.score).toBeGreaterThan(0);
        expect(fuzzyMatch("series", veryLong)!.score).toBeGreaterThan(0);
    });
});

describe("highlightFuzzy — `PSM-28`, code points not code units", () => {
    const marked = (html: string) =>
        [...html.matchAll(/<mark>(.*?)<\/mark>/g)].map((m) => m[1]).join("");

    it("marks the matched characters of an ASCII label", () => {
        expect(marked(highlightFuzzy("Fourier basis", "basis"))).toBe("basis");
    });
    ...
    it("marks correctly after an astral character (was off by one)", () => {
        ...
        const html = highlightFuzzy("𝔽ourier basis", "basis");
    ...
    it("marks correctly after a character whose lowercase is LONGER (was catastrophic)", () => {
        ...
        expect(marked(highlightFuzzy("İstanbul set", "set"))).toBe("set");
    });

    it("escapes the text it does not mark, including both quote forms", () => {
        const html = highlightFuzzy(`a <b> & "q" 'r'`, "zzz");
    ...
    it("returns escaped text unchanged when the query matches nothing", () => {
        expect(highlightFuzzy("Fourier basis", "zzz")).toBe("Fourier basis");
        expect(highlightFuzzy("Fourier basis", "   ")).toBe("Fourier basis");
    });
});
